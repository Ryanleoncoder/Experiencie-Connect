-- Esta migração corrige as funções de tentativas de login para tratar corretamente o caso de entrada inexistente

DROP FUNCTION IF EXISTS check_login_attempts(TEXT);
DROP FUNCTION IF EXISTS increment_login_attempts(TEXT);
DROP FUNCTION IF EXISTS clear_login_attempts(TEXT);
DROP FUNCTION IF EXISTS cleanup_login_attempts();
...
BEGIN
  ...
  IF v_entry.identifier IS NULL OR 
     v_entry.first_attempt IS NULL OR 
